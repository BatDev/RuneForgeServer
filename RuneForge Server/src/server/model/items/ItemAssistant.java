...
public class ItemAssistant {
	...
	public boolean addBankItem(int item, double amount) {
		synchronized(c) {
			...
			for (int i = 0; i < c.bankItems.length; i++) {
				if ((c.bankItems[i] == (item + 1)) && (c.bankItems[i] > 0) && (c.bankItemsN[i] < Config.MAXITEM_AMOUNT)) {
					c.bankItems[i] = (item + 1);
				if (((c.bankItemsN[i] + amount) < Config.MAXITEM_AMOUNT) && ((c.bankItemsN[i] + amount) > -1)) {
					...
					if((old + amount) - (long) Config.MAXITEM_AMOUNT > 0) {
						addBankItem(item, (old + amount) - (long) Config.MAXITEM_AMOUNT);
					}
				}
					return true;
				}
	...
	**/
	...
	public static final int[] donItems = {20139,20135,20143,20147,20151,20155,20159,20163,20167,1038,13531,13532,13533,13534,13535,13536,13537,13538,13539,13540,1040,1042,1044,1046,1048,1050,1053,1055,1057,15241,15243,15332,15333,15334,15335}; 
	...
	public int[][] getClueScroll = {{2677},{2678},{2679},{2680},{2681}};

		/**
		* method that calculates our bank value.
		* goes through all the items in bank, than multiplying it by the amount we have
		* once our loop is done, we print out the result depending on the bankValue
		* @author aintaro
		*/
		public void checkBankvalue() {
				int bankValue = 0;
				int pricePerItem = 0;
				for (int i = 0; i < c.bankItems.length;i++) {
					if (c.bankItems[i] != 0) {
						pricePerItem = (int)Math.floor(c.getShops().getItemShopValue(c.bankItems[i], 1, c.xRemoveSlot));
						bankValue += pricePerItem * c.bankItemsN[i];
					}
				}
				if(bankValue >= 10000 && bankValue < 10000000) {
						c.getDH().sendStatement("bank value : " + bankValue / 1000 + "k gp.");
				}
				else if(bankValue >= 10000000 && bankValue < 2147000000) {
						c.getDH().sendStatement("bank value : " + bankValue / 1000000  + "m gp.");
				} else {
						c.getDH().sendStatement("bank value : " + bankValue  + " gp.");
				}
			}
	...
	public void writeBonus() {
		...
		for (int i = 0; i < c.playerBonus.length; i++) {
			if (BONUS_NAMES[i][1].equals("i")) {
				if (c.playerBonus[i] >= 0) {
					send = BONUS_NAMES[i][0] + ": +" + (int)c.playerBonus[i];
				} else {
					send = BONUS_NAMES[i][0] + ": -"
							+ (int)java.lang.Math.abs(c.playerBonus[i]);
				}
			} else if(BONUS_NAMES[i][1].equals("p")) {
		        DecimalFormat df = new DecimalFormat("#.##");
				if (c.playerBonus[i] >= 0) {
					send = BONUS_NAMES[i][0]+": +"+(df.format(c.playerBonus[i]*100)) + "%";
				} else {
					send = BONUS_NAMES[i][0]+": -"+(df.format(java.lang.Math.abs(c.playerBonus[i])*100)) + "%";
				}
			}
			c.getPA().sendFrame126(send, BONUS_INDEXS[i]);
		}
...
return;
	...
	**/
	...
	public final String[][] BONUS_NAMES = {
		{"Stab", "i"}, {"Slash", "i"}, {"Crush", "i"}, {"Magic", "i"}, {"Range", "i"}, {"Stab", "i"}, {"Slash", "i"},
		{"Crush", "i"}, {"Magic", "i"}, {"Range", "i"}, {"Summoning", "i"}, {"Absorb Melee", "p"},
		{"Absorb Magic", "p"}, {"Absorb Ranged", "p"}, {"Strength", "i"}, {"Ranged Strength", "i"},
		{"Prayer", "i"}, {"Magic Damage", "p"}
	};
	public final int[] BONUS_INDEXS = {
			1675, 1676, 1677, 1678, 1679, 1680, 1681,
			1682, 1683, 1684, 19157, 19158, 19159, 
			19160, 1686, 19156, 1687, 19161
		};
	...
	**/
	...
	public void sendWeapon(int Weapon, String WeaponName) {
		...
		if (WeaponName.equals("Unarmed")) {
            ...
            } else if (WeaponName2.toLowerCase().contains("maul") || WeaponName.endsWith("warhammer") || WeaponName.endsWith("Tzhaar-ket-om")) {
		...
		} else if (WeaponName2.startsWith("dagger") || c.playerEquipment[c.playerWeapon] == 13905 || c.playerEquipment[c.playerWeapon] == 13899 || WeaponName2.contains("Staff of light") || c.playerEquipment[c.playerWeapon] == 15241 || c.playerEquipment[c.playerWeapon] == 18349 || c.playerEquipment[c.playerWeapon] == 10887 || WeaponName2.contains("sword")) {
	...
	**/
	public boolean is2handed(String itemName, int itemId) {
		...
		if(itemName.contains("longbow") || itemName.contains("shortbow") || itemName.contains("ark bow") || itemName.contains("Zamorak bow") || itemName.contains("Guthix bow") || itemName.contains("Saradomin bow")) {
			return true;
		}
...
return false;
	...
	if (wearID > 15085 && wearID < 15102){
		...
		synchronized(c) {
			...
			if(c.playerItems[slot] == (wearID+1)) {				
				...
				if(itemName.contains("cloak") || itemName.contains("attractor") || itemName.contains("accumulator") || itemName.contains("cape") || itemName.contains("Cape") || itemName.contains("cloak") || itemName.contains("Cloak") || itemType(wearID).equalsIgnoreCase("cape")) {
					targetSlot=1;
				} else if(itemType(wearID).equalsIgnoreCase("hat") && !itemName.contains("hatchet") || itemName.contains("Pernix cowl") || itemName.contains("cap") && !itemName.contains("cape") || itemName.contains("mask") || itemName.contains("headdress") || itemName.contains("cavalier") || itemName.contains("Cavalier") || itemName.contains("sallet") || itemName.contains("Sallet") || itemName.contains("Hat") && !itemName.contains("hatchet") || itemName.contains("mask") || itemName.contains("Mask") || itemName.contains("hat") && !itemName.contains("hatchet") || itemName.contains("helm") || itemName.contains("Helm") || itemName.contains("hood") || itemName.contains("Hood") || itemName.contains("berret") || itemName.contains("afro")) {
					targetSlot=0;
				} else if(itemType(wearID).equalsIgnoreCase("amulet") || itemName.contains("Amulet") || itemName.contains("amulet") || itemName.contains("necklace") || itemName.contains("Necklace")) {
					targetSlot=2;
				} else if(itemType(wearID).equalsIgnoreCase("arrows") || itemType(wearID).equalsIgnoreCase("arrow") && !itemName.contains("morrigan") || itemName.contains("bolt") || itemName.contains("Bolt") || itemName.contains("arrow") || itemName.contains("Arrow")) {
					targetSlot=13;
				} else if(itemType(wearID).equalsIgnoreCase("body") || itemName.contains("platemail") || itemName.contains("Pernix body") || itemName.contains("robe top") || itemName.contains("chest") && !itemName.contains("anchor") || itemName.contains("Torso") || itemName.contains("platebody") || itemName.contains("Platebody") || itemName.contains("chain") && !itemName.contains("skirt")  || itemName.contains("Chain") || itemName.contains("Top") || itemName.contains("Hauberk") || itemName.contains("hauberk") || itemName.contains("top") || itemName.contains("torso") || itemName.contains("shirt") || itemName.contains("Shirt") || itemName.contains("body") || itemName.contains("Body") || itemName.equalsIgnoreCase("Ghostly robe")) {
					targetSlot=4;
				} else if(itemType(wearID).equalsIgnoreCase("shield") || itemName.contains("Frost") || itemName.contains("Tome") || itemName.contains("frost") || itemName.contains("tome") || itemName.contains("Book") || itemName.contains("Toktz-xil-ul") || itemName.contains("book") || itemName.contains("Shield") || itemName.contains("shield") || itemName.contains("defender") || itemName.contains("Defender")) {
					targetSlot=5;
				} else if(itemType(wearID).equalsIgnoreCase("legs") || itemName.contains("Cuisse") || itemName.contains("Pernix chaps") || itemName.contains("robe legs") || itemName.contains("cuisse") || itemName.contains("chaps") || itemName.contains("Chaps") || itemName.contains("legs") || itemName.contains("Legs") || itemName.contains("Bottom") || itemName.contains("bottom") || itemName.contains("skirt") || itemName.contains("Skirt") || itemName.contains("shorts") || itemName.contains("Shorts") || itemName.contains("blouse") || itemName.contains("Blouse") || itemName.contains("tasset") || itemName.contains("Tasset") || itemName.contains("leg") || itemName.contains("Leg") || itemType(wearID).equalsIgnoreCase("legs") || itemName.equalsIgnoreCase("Void_knight_robe") || itemName.equalsIgnoreCase("Void knight robe") || itemName.equalsIgnoreCase("Ghostly robeskirt")) {
					targetSlot=7;
				} else if(itemType(wearID).equalsIgnoreCase("gloves") || itemName.contains("Gloves") || itemName.contains("gloves") || itemName.contains("vamb") || itemName.contains("Vamb")) {
					targetSlot=9;
				} else if(itemType(wearID).equalsIgnoreCase("boots") || itemName.contains("Boots") || itemName.contains("boots")) {
					targetSlot=10;	
				} else if(itemType(wearID).equalsIgnoreCase("ring") || itemName.contains("ring") || itemName.contains("Ring")) {
					targetSlot=12;
				} else {
					targetSlot = 3;
				}
			...
			if(c.getOutStream() != null && c != null) {
				if(c.playerEquipment[slot] > -1){
					if(addItem(c.playerEquipment[slot], c.playerEquipmentN[slot])) {
						if (c.playerEquipment[slot] == 20147 || c.playerEquipment[slot] == 20151 || c.playerEquipment[slot] == 20155 || c.playerEquipment[slot] == 20159 || c.playerEquipment[slot] == 20163 || c.playerEquipment[slot] == 20167 || c.playerEquipment[slot] == 20143 || c.playerEquipment[slot] == 20139 || c.playerEquipment[slot] == 20135)
							torvaChanged = true;
	...
	**/
	...
	public boolean hasAllLumps() { 
		return playerHasItem(14472,1) && playerHasItem(14474,1) && playerHasItem(14476,1);
	}
	public void makePlate() {
		...
		deleteItem(14476,1);
		addItem(14479,1);
		c.sendMessage("You fuse the pieces together and make a Dragon Platebody.");
	}
