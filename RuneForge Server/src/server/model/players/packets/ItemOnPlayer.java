...
public class ItemOnPlayer implements PacketType {
	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		case 962:
			...
			break;
		case 2518:
			handleTomatoes(c, itemId, playerId);
			break;
		default:
	...
	private void handleTomatoes(Client c, int itemId, int playerId) {
		Client usedOn = (Client) PlayerHandler.players[playerId];
		if (!c.getItems().playerHasItem(itemId))
			return;
			c.getItems().deleteItem(itemId, 1);
			c.getPA().ThrowTomatoes(c);
			usedOn.sendMessage("You got hit by a rotten tomatoe");
			c.sendMessage("You hit the reject with a trusty rotten tomatoe");
		}
	}